...
-- create a table named retirement_info
SELECT first_name, last_name
...
-- see that the retirement_ifo table looks like
...
-- Joining departments and dept_manager tables using INNER JOIN
-- redoing this blocj=k with aliases
SELECT d.dept_name,
      dm.emp_no,
     dm.from_date,
     dm.to_date
...
-- join using Left Join for retirement_info and dept_emp tables into current_emp table
SELECT ri.emp_no,
    ri.first_name,
    ri.last_name,
de.to_date
...
-- Employee count by department number, grouping by dept_no
SELECT COUNT(ce.emp_no), de.dept_no
FROM current_emp as ce
LEFT JOIN dept_emp as de
ON ce.emp_no = de.emp_no
